...
void main() {
    ...
    //vec4 p = vec4(vPosition / 5000000., time * 0.01);
    vec4 p = vec4(vPosition * 6., time * 0.01);
    float noisy = fbm(p); //snoise(vec4(vPosition / 5000000., time * 0.5));
    //gl_FragColor = vec4(sphereColor, 1.0);

    //vec4 p1 = vec4(vPosition / 3000000., time * 0.1);
    vec4 p1 = vec4(vPosition * 2., time * 0.1);
    float spots = max(snoise(p1), 0.);
    gl_FragColor = vec4(noisy, 0.0, 0.0, 1.);
